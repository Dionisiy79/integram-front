...
class SmartQ { 
                ...
                if(json.columns[i].id){
                    ...
                    if(name.substr(-2)==='ID' && zis.columns[name.substr(0,name.length-2)]){
                        ...
                        if(zis.columns[name+'ID'])
                            zis.ids[i]=1;
                        h+='<th id="'+type+'"'+(zis.ids[i]?' is-object="1"':'')+'><a sq-order desc col-id="'+json.columns[i].id+'" onclick=".orderBy(this)">'
    ...
    drawFoot(json,el,zis=this){
        ...
        if('totals' in json.columns[0]){
            ...
            for(j in json.columns)
                if(zis.ids[j]!==0)
                    h+='<th footer-id="'+json.columns[j].type+'" align="right">'+zis.formatNum(json.columns[j].totals||'',2);
    ...
    saveInlineFile(el){
        ...
        if(el.files[0].name){
            ...
            newApi_('POST','_m_set/'+$(el).closest('tr').children(':first').attr('obj-id')+'?JSON',this.saveInlineFileDone,fd,td,this);
    ...
    deleteFile(el){
        newApi_('POST','_m_set/'+$(el).closest('tr').children(':first').attr('obj-id')+'?JSON',this.deleteFileDone,'t'+$(el).closest('td').attr('req-id'),el,this);
    }
